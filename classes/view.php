<?php namespace CODERS\AirBox;
/**
 * 
 */
abstract class View{
    /**
     * @var \CODERS\AirBox\Model
     */
    private $_model = null;
    /**
     * @var string
        ...
        switch( TRUE ){
            case $name === 'action':
                if(is_array($arguments) && count($arguments)){
                    ...
                    $url = \CODERS\AirBox\Request::url($action);
    ...
    public static final function assetName( $url , $module = '' ){
        ...
        return strlen($module) ?
            sprintf('%s-%s-%s', \AirBox::ENDPOINT , $module, $file_name[0]) :
            implode('-', $file_name);
    }
    /**
    ...
    public static final function attachScripts( $script_list , $module = '' ){
        ...
        add_action( $action_hook , function() use( $script_list , $module ){
            foreach( $script_list as $script => $deps ){
                $script_url = \CODERS\AirBox\View::assetUrl( $script , $module );
                $script_name = \CODERS\AirBox\View::assetName($script , $module );
    ...
    public static final function attachStyles( $style_list = array() , $module = '' ){
        ...
        add_action( $action_hook , function() use( $style_list , $module){
            foreach( $style_list as $style ){
                $style_url = \CODERS\AirBox\View::assetUrl( $style , $module);
                $style_name = \CODERS\AirBox\View::assetName($style,$module);
    ...
    /**
     * @param \CODERS\AirBox\Model $model
     * @return \CODERS\AirBox\View
     */
    ...
    public static final function create( $request ){
        ...
        $class = sprintf('\CODERS\AirBox\%s\%sView',$module,$view);
